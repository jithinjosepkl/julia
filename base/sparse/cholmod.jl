...
import Base.LinAlg: (\), A_mul_Bc, A_mul_Bt, Ac_ldiv_B, Ac_mul_B, At_ldiv_B, At_mul_B,
                 A_ldiv_B!, cholfact, cholfact!, det, diag, full, ishermitian, isposdef,
...
for Ti in IndexTypes
    @eval begin
        ...
        function allocate_sparse(nrow::Integer, ncol::Integer, nzmax::Integer, sorted::Bool, packed::Bool, stype::Integer, ::Type{Float64}, ::Type{$Ti})
            s = Sparse(ccall((@cholmod_name "allocate_sparse" $Ti
                , :libcholmod), Ptr{C_Sparse{Float64,$Ti}},
        ...
        function allocate_sparse(nrow::Integer, ncol::Integer, nzmax::Integer, sorted::Bool, packed::Bool, stype::Integer, ::Type{Complex{Float64}}, ::Type{$Ti})
            s = Sparse(ccall((@cholmod_name "allocate_sparse" $Ti
                , :libcholmod), Ptr{C_Sparse{Complex{Float64},$Ti}},
                    (Csize_t, Csize_t, Csize_t, Cint,
                        Cint, Cint, Cint, Ptr{Void}),
                    nrow, ncol, nzmax, sorted,
        ...
        function free_factor{Tv<:VTypes}(ptr::Ptr{C_Factor{Tv,$Ti}})
            @isok ccall((@cholmod_name "free_factor" $Ti
                , :libcholmod), Cint,
        ...
        function aat{Tv<:VRealTypes}(A::Sparse{Tv,$Ti}, fset::Vector{$Ti}, mode::Integer)
            s = Sparse(ccall((@cholmod_name "aat" $Ti
        ...
        function copy{Tv<:VRealTypes}(A::Sparse{Tv,$Ti}, stype::Integer, mode::Integer)
            s = Sparse(ccall((@cholmod_name "copy" $Ti
                , :libcholmod), Ptr{C_Sparse{Tv,$Ti}},
        ...
        function sparse_to_dense{Tv<:VTypes}(A::Sparse{Tv,$Ti})
            d = Dense(ccall((@cholmod_name "sparse_to_dense" $Ti
        ...
        function dense_to_sparse{Tv<:VTypes}(D::Dense{Tv}, ::Type{$Ti})
            s = Sparse(ccall((@cholmod_name "dense_to_sparse" $Ti
                ,:libcholmod), Ptr{C_Sparse{Tv,$Ti}},
                    (Ptr{C_Dense{Tv}}, Cint, Ptr{UInt8}),
                        D.p, true, common($Ti)))
            finalizer(s, free!)
        ...
        function factor_to_sparse!{Tv<:VTypes}(F::Factor{Tv,$Ti})
            ...
            s = Sparse(ccall((@cholmod_name "factor_to_sparse" $Ti
                ,:libcholmod), Ptr{C_Sparse{Tv,$Ti}},
        ...
        function speye{Tv<:VTypes}(m::Integer, n::Integer, ::Type{Tv})
            s = Sparse(ccall((@cholmod_name "speye" $Ti
                , :libcholmod), Ptr{C_Sparse{Tv,$Ti}},
                    (Csize_t, Csize_t, Cint, Ptr{UInt8}),
        ...
        function spzeros{Tv<:VTypes}(m::Integer, n::Integer, nzmax::Integer, ::Type{Tv})
            s = Sparse(ccall((@cholmod_name "spzeros" $Ti
                , :libcholmod), Ptr{C_Sparse{Tv,$Ti}},
        ...
        function transpose{Tv<:VTypes}(A::Sparse{Tv,$Ti}, values::Integer)
            s = Sparse(ccall((@cholmod_name "transpose" $Ti
                ,:libcholmod), Ptr{C_Sparse{Tv,$Ti}},
        ...
        function copy_factor{Tv<:VTypes}(F::Factor{Tv,$Ti})
            f = Factor(ccall((@cholmod_name "copy_factor" $Ti
        ...
        function copy_sparse{Tv<:VTypes}(A::Sparse{Tv,$Ti})
            s = Sparse(ccall((@cholmod_name "copy_sparse" $Ti
                ,:libcholmod), Ptr{C_Sparse{Tv,$Ti}},
                    (Ptr{C_Sparse{Tv,$Ti}}, Ptr{UInt8}),
                        A.p, common($Ti)))
            finalizer(s, free!)
        ...
        function ssmult{Tv<:VRealTypes}(A::Sparse{Tv,$Ti}, B::Sparse{Tv,$Ti}, stype::Integer, values::Bool, sorted::Bool)
            lA = unsafe_load(A.p)
            lB = unsafe_load(B.p)
            lA.ncol == lB.nrow || throw(DimensionMismatch("inner matrix dimensions do not fit"))
            s = Sparse(ccall((@cholmod_name "ssmult" $Ti
                ,:libcholmod), Ptr{C_Sparse{Tv,$Ti}},
        ...
        function horzcat{Tv<:VRealTypes}(A::Sparse{Tv,$Ti}, B::Sparse{Tv,$Ti}, values::Bool)
            s = Sparse(ccall((@cholmod_name "horzcat" $Ti
                , :libcholmod), Ptr{C_Sparse{Tv,$Ti}},
        ...
        function scale{Tv<:VRealTypes}(S::Dense{Tv}, scale::Integer, A::Sparse{Tv,$Ti})
            ...
            if scale == SCALAR
                sS.nrow == 1 || throw(DimensionMismatch("scaling argument must have length one"))
            elseif scale == ROW
                sS.nrow*sS.ncol == sA.nrow || throw(DimensionMismatch("scaling vector has length $(sS.nrow*sS.ncol), but matrix has $(sA.nrow) rows."))
            elseif scale == COL
                sS.nrow*sS.ncol == sA.ncol || throw(DimensionMismatch("scaling vector has length $(sS.nrow*sS.ncol), but matrix has $(sA.ncol) columns"))
            elseif scale == SYM
                sA.nrow == sA.ncol || throw(DimensionMismatch("matrix must be square"))
                sS.nrow*sS.ncol == sA.nrow || throw(DimensionMismatch("scaling vector has length $(sS.nrow*sS.ncol), but matrix has $(sA.ncol) columns and rows"))
            end
        ...
        function vertcat{Tv<:VRealTypes}(A::Sparse{Tv,$Ti}, B::Sparse{Tv,$Ti}, values::Bool)
            s = Sparse(ccall((@cholmod_name "vertcat" $Ti
                , :libcholmod), Ptr{C_Sparse{Tv,$Ti}},
                    (Ptr{C_Sparse{Tv,$Ti}}, Ptr{C_Sparse{Tv,$Ti}}, Cint, Ptr{UInt8}),
                        A.p, B.p, values, common($Ti)))
            finalizer(s, free!)
        ...
        function solve{Tv<:VTypes}(sys::Integer, F::Factor{Tv,$Ti}, B::Dense{Tv})
            size(F,1) == size(B,1) || throw(DimensionMismatch("LHS and RHS should have the same number of rows. LHS has $(size(F,1)) rows, but RHS has $(size(B,1)) rows."))
            d = Dense(ccall((@cholmod_name "solve" $Ti
                ,:libcholmod), Ptr{C_Dense{Tv}},
        ...
        function spsolve{Tv<:VTypes}(sys::Integer, F::Factor{Tv,$Ti}, B::Sparse{Tv,$Ti})
            size(F,1) == size(B,1) || throw(DimensionMismatch("LHS and RHS should have the same number of rows. LHS has $(size(L,1)) rows, but RHS has $(size(B,1)) rows."))
            s = Sparse(ccall((@cholmod_name "spsolve" $Ti
                ,:libcholmod), Ptr{C_Sparse{Tv,$Ti}},
...
function read_sparse(file::CFILE)
    s = Sparse(ccall((:cholmod_read_sparse, :libcholmod), Ptr{C_SparseVoid},
        (Ptr{Void}, Ptr{UInt8}),
...
function convert{Tv,Ti}(::Type{SparseMatrixCSC{Tv,Ti}}, A::Sparse{Tv,Ti})
    s = unsafe_load(A.p)
    B = SparseMatrixCSC(s.nrow, s.ncol, increment(pointer_to_array(s.p, (s.ncol + 1,), false)), increment(pointer_to_array(s.i, (s.nzmax,), false)), copy(pointer_to_array(s.x, (s.nzmax,), false)))
    if s.stype != 0
        return isreal(A) ? Symmetric(B, s.stype < 0 ? :L : :U) : Hermitian(B, s.stype < 0 ? :L : :U)
    end
    return B
end
...
function size(F::Factor, i::Integer)
    i < 1 && error("arraysize: dimension out of range")
    s = unsafe_load(F.p)
    if i <= 2
        return int(s.n)
    end
...
A_mul_Bt(A::Sparse, B::Sparse) = A_mul_Bc(A, B) # in the unlikely event of writing A*B.' instead of A*B'

At_mul_B(A::Sparse, B::Sparse) = Ac_mul_B(A,B) # in the unlikely event of writing A.'*B instead of A'*B
